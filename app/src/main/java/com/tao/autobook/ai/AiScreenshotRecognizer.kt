...
class AiScreenshotRecognizer {
    ...
    private fun buildRequest(model: String, imageBase64: String, ocrText: String, mimeType: String = "image/jpeg"): JSONObject {
        val prompt = """
                            ...
                            3b. 能判断出细项时，categoryHint 必须填最细的二级分类，不要只填一级：
                               - 电费/供电/国网/电力公司/购电/电费充值 → 电费（不要只写生活缴费）
                               - 水费/自来水/供水/水务 → 水费
                               - 燃气/天然气/煤气 → 燃气
                               - 房租/租金 → 房租
                               - 宽带/网费/话费/手机充值/电信/联通/移动 → 网费
                               - 加油/中石化/中石油/充电桩/换电 → 加油
                               - 打车/滴滴/网约车/出租车 → 打车；地铁/轨道交通 → 地铁；公交 → 公交；停车 → 停车
                               - 早餐/午餐/晚餐/外卖/零食/水果 → 对应餐饮二级分类
                               - 游戏/点券/皮肤 → 游戏；电影/影城 → 电影；健身/球馆 → 运动
                               - 服装/鞋/衣服 → 服装；纸巾/日用/清洁 → 日用品；手机/电脑/耳机/数码 → 数码
                               判不出细项时才退回一级分类
...
$truncated"""
    ...
    companion object {
        ...
        fun parseAiJson(jsonText: String): AiParsedPayment? = runCatching {
            ...
            // 二级分类名也要算进来，避免 note 被写成「电费」「打车」这种分类名
            val cats = setOf(
                "购物","餐饮","交通","生活缴费","娱乐","医疗","教育","转账","人情","退款","工资","奖金","理财","其他","宠物",
                "电费","水费","燃气","房租","网费","加油","打车","地铁","公交","停车",
                "早餐","午餐","晚餐","外卖","零食","水果","游戏","电影","运动","服装","日用品","数码"
            )
