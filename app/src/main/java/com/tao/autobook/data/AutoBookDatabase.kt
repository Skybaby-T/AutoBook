...
abstract class AutoBookDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE TABLE IF NOT EXISTS budgets (categoryId TEXT PRIMARY KEY NOT NULL, amountCents INTEGER NOT NULL, updatedAt INTEGER NOT NULL DEFAULT 0)")
            }
        }

        private val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE transactions ADD COLUMN excludeFromStats INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE transactions ADD COLUMN excludeFromBudget INTEGER NOT NULL DEFAULT 0")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_transactions_excludeFromStats ON transactions(excludeFromStats)")
            }
        }
